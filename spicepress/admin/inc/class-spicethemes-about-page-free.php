...
if (!class_exists('SpicePress_About_Page')) {
	...
	public function style_and_scripts( $hook_suffix ) {
		...
		if ( 'toplevel_page_spicepress-welcome' == $hook_suffix || 'admin_page_spice-settings-importer' == $hook_suffix || 'themes.php' == $hook_suffix) {


			wp_enqueue_style( 'spicepress-info-screen-free-css', ST_TEMPLATE_DIR_URI . '/admin/assets/css/welcome-free.css' );
			...
			wp_enqueue_script('plugin-install-free');
			...
			wp_enqueue_script('spicepress-ajax', ST_TEMPLATE_DIR_URI . '/admin/assets/js/ajax-free.js', array('jquery'));
	...
	public function scripts_for_customizer() {
		...
		public function add_theme_info_menu() {
			...
			add_menu_page(sprintf(esc_html__('Welcome to %1$s %2$s', 'spicepress'), esc_html($theme->get('Name')), esc_html($theme->get('Version'))), $title, 'edit_theme_options', 'spicepress-welcome', array($this, 'print_welcome_page'));
		}
